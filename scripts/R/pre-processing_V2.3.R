...
for(i in 1:length(seu.list.noDB.norm)){
  ...
  #seu.list.norm[[i]] <- subset(seu.list.norm[[i]], subset = percent.mt < 25)
  seu.list.noDB.norm[[i]] <- SCTransform(seu.list.noDB.norm[[i]], vars.to.regress = "percent.mt", verbose = FALSE)
  seu.list.noDB.norm[[i]] <- RunPCA(seu.list.noDB.norm[[i]], npcs=30, verbose=TRUE)
...
###################       NORMALIZING DATA   SCTransform()    ###############################
obj.name <- paste0("noDub.norm.SCTransform.",proj.name,".list.",today,".rds")
saveRDS(seu.list.noDB.norm, file=paste(output.path,obj.name,sep="/") , 
        ascii = FALSE, version = NULL,compress = TRUE, refhook = NULL)
###################       VISUALIZATION   NO DB    ##################################
seu_plots(seu.obj.list=seu.list.noDB.norm, output.path =output.path.plots, processing.stage="02_NORM.NoDub_SCTransform")
###################       VISUALIZATION   NO DB    ##################################
#seu.list.noDB.norm <- readRDS("/Volumes/MarkIV/178_sc_Gebaur/03_QC/11.04.2024/noDub.norm.scFFPE_Gebaur.list.11.04.2024.rds")
...
######################      IDENTIFY LOW QUALITY CELLS  METHOD 1      ##################################
...
for (nm in names(seu.list.noDB.norm.qc)){
  # Example usage:
  out[[nm]] <- analyze_scatterplot(seu.list.noDB.norm.qc[[nm]],probs = 0.1,model_name = "polyreg")
  
  Sample_QC_parameter <- data.frame("nCount_RNA_High"=out[[nm]]$threshold$nCount_RNA,
                                    "nFeature_RNA_High"=out[[nm]]$threshold$nFeature_RNA)
  #Save the plots 
  plots[[nm]] <- seu_plots_withThresholdLines_method1(seu.obj = seu.list.noDB.norm.qc[[nm]],qc_parameter_threshold = Sample_QC_parameter)
  
  #Subseting sample
  message(sprintf("Subseting sample with setted parameters..."))
  seu.list.noDB.norm.qc[[nm]] <- subset(seu.list.noDB.norm.qc[[nm]], subset= nCount_RNA < Sample_QC_parameter$nCount_RNA_High )
  seu.list.noDB.norm.qc[[nm]] <- subset(seu.list.noDB.norm.qc[[nm]], subset= nFeature_RNA < Sample_QC_parameter$nFeature_RNA_High )
  #Saving QC parameters to misc
  seu.list.noDB.norm.qc[[nm]]@misc <- list("QC_Parameters_auto"=list(
    "nCount_RNA"= paste0("X < ", Sample_QC_parameter$nCount_RNA_High),
    "nFeature_RNA"=paste0("X < ", Sample_QC_parameter$nFeature_RNA_High)
  )
  )
  
}
name <- paste0("03_NORM.NoDub_SCTranscorm_Pre-processed_auto", "_ViolinPlot_Number_Cell.MT.Features_allSamples.pdf")
